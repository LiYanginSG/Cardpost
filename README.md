...
- Vercel cron (hourly) flips arrived cards to delivered and sends the one email this app sends
...
5. The cron in `vercel.json` runs `/api/cron/deliver` hourly. Vercel sends the `CRON_SECRET` bearer header itself.
